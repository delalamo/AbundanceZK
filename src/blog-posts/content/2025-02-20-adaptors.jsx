...
// MathJax import remains if needed, removed for brevity here if not used in this specific component's content
...
export const post = {
  ...
  content: (() => {
    ...
    const ContentComponent = () => {
      ...
      useEffect(() => {
        ...
        // This ensures list numbers match the reference numbers exactly
        ...
        footnoteListItems.forEach(item => {
            ...
            if (footnoteMap.hasOwnProperty(itemId)) {
                // Add the number before the list item content
                const numberSpan = document.createElement('span');
                numberSpan.className = 'footnote-list-number'; // Class for styling
                numberSpan.textContent = `${footnoteMap[itemId]}. `;
                item.insertBefore(numberSpan, item.firstChild);
            }
        });
        // Prevent running the list numbering part multiple times on re-renders
        // A more robust solution might check if the numberSpan already exists
        // Or structure the effect cleanup properly depending on dependencies.
        // For simple static content, this might be okay, but beware of HMR.
        // A simple flag could work too:
      ...
      return (
        ...
        <div className="content-container" ref={contentRef}>
          ...
          {/* Structure remains same: className, empty <a>, correct href/id */}
          ...
          <p style={{ textAlign: 'center', fontSize: '0.9em' }}>
                <span style={{ fontStyle: 'italic' }}>Figure from </span><sup className="footnote-ref">[<a href="#fn6" id="fnref6_fig1"></a>]</sup>
          </p>
          ...
          />
          <p style={{ textAlign: 'center', fontSize: '0.9em' }}>
              <span style={{ fontStyle: 'italic' }}>Figure from </span><sup className="footnote-ref">[<a href="#fn6" id="fnref6_fig2"></a>]</sup>
          </p>
          ...
          <section className="footnotes-section">
            <hr />
            <h2>Notes</h2>
            {/* Changed: Removed default <ol> numbering via CSS is safer now */}
  ...
  })(), // Immediately invoke the function to get the JSX element
};